...
ga = GA(func=fitness, n_dim=6, size_pop=10, max_iter=50, prob_mut=0.001, lb=[1, 1, 1, 0.001, 1, 1], ub=[200, 200, 14, 0.1, 500, 128], precision=1e-7)
_, best_y, generation_best_X, generation_best_Y, all_history_X, all_history_Y = ga.run()
print('\n', 'best_y:', best_y)

time_flag = str(time.strftime("%m-%d_%H:%M:%S", time.localtime()))
fitness_flag = "fitness:" + str(round(best_y[-1], 3)) + "_"
...
savemat(data_flag, {'runtime': run_time,
                    'history_position': generation_best_X,
                    'history_min_fitness': generation_best_Y,
					'all_history_X': all_history_X,
					'all_history_Y': all_history_Y})