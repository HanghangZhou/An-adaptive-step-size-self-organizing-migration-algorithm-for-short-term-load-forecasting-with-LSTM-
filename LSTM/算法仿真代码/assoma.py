...
def logistic_map(number, u=4):
    log_set = numpy.zeros(number)
    log_set[0] = numpy.random.uniform()

    for i in numpy.arange(1, number):
...
def generate_population(size, min_s, max_s, dimensions, benchmark):
    X = numpy.zeros((size, dimensions))
    population = list()
    for j in range(dimensions):
        # logistic混沌映射，针对整个粒子群来看，每一个维度都是均匀混沌的
        log_set = logistic_map(size)
        for i in range(size):
            if j == 0:
    ...
    for i in range(size):
        fitness = benchmark(X[i, :])
        pop = Individual(X[i, :], fitness)
        ...
        for index, individual in enumerate(population):
            ...
            while t < path_length:
                ...
                if len(Goodstep) == 0:
                    ...
                    size = len(Goodstep)
                    good_ustep = sum(Goodstep) / size
                    ustep = (1 - c) * ustep + c * good_ustep
                    step = float(abs(numpy.random.normal(ustep, 0.01, 1)))
...
def run_algorithm(dimensions, min_border, max_border, benchmark):
    ...
    winner, history_fitness_list, history_position_list, history_min_fitness_list = soma_all_to_one \
        (population, prt, path_length, step, fes, min_s, max_s, dimensions, benchmark)
    print()
