    ...
    while fes_iteration < fes:  # check fes
        if numpy.random.random() < 0.5:
            ...
            for index, individual in enumerate(population):
                ub = numpy.array(max_s)
                lb = numpy.array(min_s)
                current_position = individual.params
                # 反向个体
                opposition_position = ub + lb - current_position
                fitness = benchmark(opposition_position)
                ...
                if fitness < individual.fitness:
                    print("oppo" , end=" ")
                    individual.params = opposition_position
