...
%             [best_value, best_index] = sort(fitness);
%             pbest_index = best_index(1:(N*0.1));
%             random_num = pbest_index(randi(numel(pbest_index),1,1));
%             leader = x(random_num,:);%将种群的第lead_pos个个体作为领导者           
            strt_pop=x(i,:);
