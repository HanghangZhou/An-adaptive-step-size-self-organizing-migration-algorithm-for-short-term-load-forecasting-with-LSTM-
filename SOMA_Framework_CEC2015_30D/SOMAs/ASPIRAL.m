...
function [RunResult,RunValue,RunTime,RunFES,RunOptimization,RunParameter]=ASPIRAL(problem,N,runmax)
'ASPIRAL'
...
for run=1:runmax
    ...
    TempFES=FESMAX;
    t1=clock;
    x=C_Initpop(N,D,lu);%种群初始化，参考CEP
...
%                 end
            ...
            while t<pathLength
                ...
                for j=1:D
                    v(i,j) = strt_pop(j) + abs( leader(j) - strt_pop(j) ) * ( exp(1)^b*l ) * cos(2 * pi * l) * t * prt_vector(j);
                end
